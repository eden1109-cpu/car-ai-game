(() => {
  ...
  function loadSave() {
    state.best = Number(getStorage(STORAGE_KEYS.best, 0) || 0);
    state.score = Number(getStorage(STORAGE_KEYS.score, 0) || 0);
    state.coins = Number(getStorage(STORAGE_KEYS.coins, 0) || 0);
    try {
      const skins = JSON.parse(getStorage(STORAGE_KEYS.skins, '[\"classic\"]'));
      state.owned = new Set(Array.isArray(skins) ? skins : ['classic']);
    } catch {
      state.owned = new Set(['classic']);
    }
    state.activeSkin = getStorage(STORAGE_KEYS.active, 'classic') || 'classic';
    if (!state.owned.has(state.activeSkin)) state.activeSkin = 'classic';
  }

  function save() {
    setStorage(STORAGE_KEYS.best, String(state.best));
    setStorage(STORAGE_KEYS.score, String(Math.floor(state.score)));
    setStorage(STORAGE_KEYS.coins, String(state.coins));
    setStorage(STORAGE_KEYS.skins, JSON.stringify([...state.owned]));
    setStorage(STORAGE_KEYS.active, state.activeSkin);
  }
  ...
  function showFx(text, bg = '#1f8d4f') {
    el.fxLabel.style.boxShadow = '0 0 18px ' + bg + 'aa';
  ...
  function pixelCar(x, y, w, h, color, enemy = false) {
    const px = Math.max(2, Math.floor(w / 9));
    ctx.save();
    ctx.translate(x - w / 2, y - h / 2);

    ctx.fillStyle = color;
    ctx.fillRect(px, h * 0.08, w - px * 2, h * 0.84);

    ctx.fillStyle = enemy ? '#ffccd8' : '#bff3ff';
    ctx.fillRect(px * 2, h * 0.2, w - px * 4, h * 0.18);

    ctx.fillStyle = '#132230';
    ctx.fillRect(px * 2, h * 0.46, w - px * 4, h * 0.22);
    ...
    ctx.fillRect(px * 2, h * 0.12, w - px * 5, px);
    ctx.restore();
  }
  ...
  function render() {
    ...
    grad.addColorStop(0, '#0f1f3e');
    grad.addColorStop(0.55, '#0a1328');
    grad.addColorStop(1, '#070d18');
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, state.W, state.H);

    // neon side glow
    ctx.fillStyle = '#ff3bf322';
    ctx.fillRect(state.roadX - 8, 0, 8, state.H);
    ctx.fillStyle = '#24deff22';
    ctx.fillRect(state.roadX + state.roadW, 0, 8, state.H);

    ctx.fillStyle = '#182436';
    ctx.fillRect(state.roadX, 0, state.roadW, state.H);

    ctx.strokeStyle = 'rgba(110, 239, 255, 0.6)';
    ctx.lineWidth = 3;
    ...
    for (let i = 1; i < state.LANES; i += 1) {
      ...
      ctx.strokeStyle = i % 2 ? 'rgba(109,247,255,0.33)' : 'rgba(255,99,241,0.22)';
      ctx.beginPath();
      ctx.moveTo(x, 0);
    ...
    state.coinItems.forEach((c) => {
      const cg = ctx.createRadialGradient(c.x - 3, c.y - 3, 2, c.x, c.y, c.r + 4);
      cg.addColorStop(0, '#fff8bf');
      cg.addColorStop(0.55, '#ffd34d');
      cg.addColorStop(1, '#a06d00');
      ctx.fillStyle = cg;
      ctx.beginPath();
